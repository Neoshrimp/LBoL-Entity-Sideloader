...
namespace LBoLEntitySideloader.TemplateGen
{
    ...
    public abstract class TemplateGen<ED> where ED : EntityDefinition
    {
        ...
        protected CodeTypeDeclaration InnitDefintionType(IdContainer Id, bool overwriteVanilla = false)
        {
            ...
            {
                Log.log.LogError($"{typeof(ED)} template gen: type {MakeDefName(Id)} was already generated.");
                return null;
            }
        ...
        {
            if (UniqueTracker.Instance.gennedAssNames.Contains(newAssName))
            {
                Log.log.LogWarning($"FinalizeGen: {newAssName} was already generated by {originAssembly.GetName().Name}. Skipping generation. FinalizeGen should only be called once.");
                return;
            }

            UniqueTracker.Instance.gennedAssNames.Add(newAssName);

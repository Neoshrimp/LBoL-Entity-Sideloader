...
/// credit goes to https://codeberg.org/matiaslavik/unity-async-textureimport
namespace LBoLEntitySideloader.Resource.AsyncTextureImport
{
    ...
    public class TextureImporter
    {
        ...
        public TextureImporter(FREE_IMAGE_FILTER imageFilter)
        {
            this.imageFilter = imageFilter;
        }


        public static UniTask<Texture2D> LoadTextureAsync(byte[] textureData, FREE_IMAGE_FORMAT format, int mipLevels = -1)
        {
            var textureImporter = new TextureImporter();
            return UniTask.RunOnThreadPool(() => 
                textureImporter.CreateTexture(textureImporter.ImportTextureFromMemory(textureData, format, mipLevels))
                );
        }
